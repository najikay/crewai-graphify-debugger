...
### 4.4 Run Integrity, Context Isolation & Provider Configuration

| ID | Requirement |
|---|---|
| FR-15 | The active LLM backend SHALL be selected at runtime from the `LLM_PROVIDER` and `DEFAULT_MODEL` environment variables (loaded from `.env`). Supported providers: `claude` (default) and `deepseek` — **both** routed through the `ApiGatekeeper` (via `GatekeeperLLM` + a provider-specific `_ClientProtocol` adapter). Switching providers MUST require zero source changes. |
| FR-16 | The Navigator Agent SHALL receive the contents of `hot.md` injected directly via `crew.kickoff(inputs={"hot_md_content": ...})`. It MUST NOT call a tool to fetch vault context, eliminating fabricated/hallucinated file paths. |
| FR-17 | The `read_code_slice` and `apply_patch` tools SHALL resolve a requested path against `workspace/target/`, falling back to a recursive filename search when the exact relative path is absent, so partial/mismatched paths still bind to the correct on-disk file. |
| FR-18 | The Reasoner Agent SHALL emit an explicit `target_file` field in its Hypothesis JSON. The Patcher Agent SHALL take its `file_path` exclusively from that field and is FORBIDDEN from inferring, guessing, or reusing any other filename seen earlier in the run. The Patcher's task context SHALL be restricted to the Reasoner's output only. |
| FR-19 | The `apply_patch` tool SHALL reject trivial patches: any call where `original_code == new_code`, or where the stripped `original_code` is shorter than the minimum patch length, MUST be refused with an error and MUST NOT modify any file. |
| FR-20 | After a successful (non-SKIPPED) run, the system SHALL physically diff `workspace/target/broken-python/` against the pristine fixture. If no files changed it SHALL emit a VALIDATION FAILED signal; otherwise it SHALL copy the target tree to `workspace/results/run_<YYYYMMDD_HHMMSS>/` and write a `run_summary.txt` containing the changed-file list, a unified diff, and the agent's final answer. |
...
| **150-Line Limit** | No source file (excluding tests) may exceed 150 lines; enforced by `scripts/check_line_limits.py` and the CI gate. |
| **96% Test Coverage** | `pytest-cov` must report ≥ 96% branch coverage; CI fails below this threshold. (`fail_under` floor is 90; the working target is > 96%.) |
...
3. The system supports multiple LLM providers selected at runtime via the `LLM_PROVIDER` env toggle (see FR-15): Anthropic Claude (default) and DeepSeek. **Both** providers route through the `ApiGatekeeper`, so budget enforcement and token telemetry apply uniformly regardless of provider.
...
### 8.2 Technical Debt — DeepSeek ו-ApiGatekeeper (נפתר ✅)

החוב **נפתר**. בעבר DeepSeek עקף את ה-`ApiGatekeeper`; כעת `_make_llm()` מחזיר
`GatekeeperLLM` עבור שני ה-providers, ו-`DeepSeekAdapter` (ב-`sdk/deepseek_client.py`)
מנתב קריאות DeepSeek דרך ה-gatekeeper עם token usage אמיתי. `deepseek-chat` נוסף
ל-pricing וב-allow-list, כך שדרישות **FR-10…FR-14** (אכיפת gatekeeper, budget,
ו-telemetry) תקפות כעת עבור **כל** ה-providers — לא רק Claude. ראו PLAN §10.2.
