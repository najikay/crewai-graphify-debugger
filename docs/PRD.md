...
| ID | Requirement |
|---|---|
| FR-15 | The active LLM backend SHALL be selected at runtime from the `LLM_PROVIDER` and `DEFAULT_MODEL` environment variables (loaded from `.env`). Supported providers: `claude` (default, routed through the `ApiGatekeeper`) and `deepseek` (routed via `crewai.LLM`). Switching providers MUST require zero source changes. |
...
3. The system supports multiple LLM providers selected at runtime via the `LLM_PROVIDER` env toggle (see FR-15): Anthropic Claude (default, gatekeeper-routed) and DeepSeek (via `crewai.LLM`). Budget/rate telemetry through the `ApiGatekeeper` currently applies to the Claude path; gatekeeper coverage for third-party providers is a follow-up item.
